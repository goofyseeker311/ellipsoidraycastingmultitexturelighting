...
wimagefiles={"physical-world-map.jpg","Fondvulcain.jpg","world116cE.jpg"}; wimagescount=size(wimagefiles,2); wimages={}; wimagesizes=zeros(wimagescount,3);
for u=1:wimagescount; wimages{u}=double(imread(wimagefiles{u}))/255; wimagesizes(u,:)=size(wimages{u}); figure(4+u); imshow(wimages{u}); endfor;
lightvalue=3.5; pointlightscount = 5; pointlights = [100*(rand(pointlightscount,3)-0.5) ones(pointlightscount,3)]; #randi(255,pointlightscount,3)/255
nspheres = 100; scenespheres = [100*(rand(nspheres,3)-0.5) 6*rand(nspheres,1)+4 2*rand(nspheres,3)+0.5 randi(255,nspheres,3)/255 randi(wimagescount,nspheres,1)]; #ones(nspheres,1)
...
cki=find(ck(:,2)>(nspheres-pointlightscount)); ckicount=size(cki,1); ckni=ckn(cki,:); kcni=kci(cki,:); pld3n=pld3(cki,:);
